    ...
    for nid in ids:
        ...
        if row:
            ...
            notes.append({
                ...
                "tags": tags, "word_count": row["word_count"],
...
def list_notes(
    ...
    rows = vault.db.execute(
        f"SELECT n.*, "
        f"(SELECT GROUP_CONCAT(t.tag, ',') FROM tags t WHERE t.note_id = n.id) as tag_list "
    ...
    for row in rows:
        ...
        notes.append({
            "id": row["id"], "title": row["title"], "status": row["status"],
            "tags": tag_list, "word_count": row["word_count"],
            "summary": row["summary"], "created": row["created"],
            "updated": row["updated"],
        })
...
def get_backlinks(note_id: str) -> str:
    ...
    rows = vault.db.execute(
        "SELECT l.source_id, n.title, n.path, l.line_number, l.context "
...
def get_hubs(limit: int = 20) -> str:
    ...
    rows = vault.db.execute(
        ...
        "GROUP BY l.target_id ORDER BY inbound DESC LIMIT ?",
        (limit,),
    ).fetchall()

    return json.dumps([
        {"id": r["id"], "title": r["title"], "inbound_links": r["inbound"]}
        for r in rows
    ])
...
def vault_status() -> str:
    ...
    by_status = {}
    for row in conn.execute("SELECT status, COUNT(*) as c FROM notes WHERE type NOT IN ('index') GROUP BY status"):
        by_status[row["status"]] = row["c"]

    tag_count = conn.execute("SELECT COUNT(DISTINCT tag) as c FROM tags").fetchone()["c"]
    top_tags = [
        {"tag": r["tag"], "count": r["count"]}
        for r in conn.execute("SELECT tag, COUNT(*) as count FROM tags GROUP BY tag ORDER BY count DESC LIMIT 10")
    ]
...
def lint_vault(rule: str = "") -> str:
    ...
    Args:
        rule: Specific rule to check (leave empty for all). Rules: missing-tags, missing-summary, broken-links, orphaned-notes, expired-notes, deprecated-no-successor, low-confidence
    """
    vault = _get_vault()
    vault.auto_sync()
    conn = vault.db
    ...
    issues: list[dict] = []
    rules_to_run = [rule] if rule else [
        "missing-tags", "missing-summary", "broken-links",
        "orphaned-notes", "expired-notes", "low-confidence",
    ]

    if "missing-tags" in rules_to_run:
        for row in conn.execute(
            "SELECT id, path FROM notes WHERE type NOT IN ('index','raw') "
            "AND id NOT IN (SELECT DISTINCT note_id FROM tags)"
        ):
            issues.append({"rule": "missing-tags", "severity": "warning",
                           "note_id": row["id"], "message": "No tags"})

    if "missing-summary" in rules_to_run:
        for row in conn.execute(
            "SELECT id, path FROM notes WHERE type NOT IN ('index','raw') "
            "AND (summary IS NULL OR LENGTH(TRIM(COALESCE(summary, ''))) = 0)"
        ):
            issues.append({"rule": "missing-summary", "severity": "warning",
                           "note_id": row["id"], "message": "No summary"})

    if "broken-links" in rules_to_run:
        for row in conn.execute(
            "SELECT l.source_id, l.target_ref, l.line_number "
            "FROM links l WHERE l.target_id IS NULL"
        ):
            issues.append({"rule": "broken-links", "severity": "warning",
                           "note_id": row["source_id"],
                           "message": f"Broken link: [[{row['target_ref']}]]"})

    if "orphaned-notes" in rules_to_run:
        for row in conn.execute(
            "SELECT id FROM notes WHERE type NOT IN ('index','raw') "
            "AND id NOT IN (SELECT DISTINCT target_id FROM links WHERE target_id IS NOT NULL) "
            "AND id NOT IN (SELECT DISTINCT source_id FROM links)"
        ):
            issues.append({"rule": "orphaned-notes", "severity": "info",
                           "note_id": row["id"], "message": "No links in or out"})
